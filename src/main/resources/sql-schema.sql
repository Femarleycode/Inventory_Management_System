...
CREATE TABLE if not exists ims.orders (order_id int NOT NULL UNIQUE AUTO_INCREMENT, customer_id int NOT NULL, date timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, total_price decimal(7,2) NOT NULL, PRIMARY KEY (order_id), FOREIGN KEY (customer_id) REFERENCES ims.customers (customer_id));
